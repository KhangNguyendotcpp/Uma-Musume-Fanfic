# Mr. C.B.

## 1. Định danh
- **Loại hồ sơ:** nhân vật đã được xác nhận trong nguồn.
- **Continuity:** `CINDERELLA_GRAY`

## 2. Hồ sơ cơ bản
- **Tuổi fanfic tại năm 2026:** **18** — `ORIGINAL/APPROVED`; Oguri Cap = 17 là neo continuity, không phải tuổi official.
- **Tên Nhật:** ミスターシービー; **Romaji:** Misutā Shībī; **biệt danh:** CB
- **Sinh nhật:** 7 tháng 4; **Senior Division**; **sống một mình** trong profile baseline
- **Chiều cao:** 166 cm; **số đo:** B84/W55/H80; **cỡ giày:** 25 cm; **cân nặng:** không đổi
- **Tự xưng:** Atashi; **Trainer:** Kimi
- **Seiyuu:** Yurina Amami; **solo:** `Yumehayate`

## 3. Tính cách
Free spirit cực mạnh, ghét bị ép vào khuôn và thường chạy theo cảm giác/điều mình đã tự đồng ý. Có thể hơi clumsy trong đời thường nhưng racing presence rất lớn. Không nên biến sự tự do thành irresponsibility tuyệt đối; CB có logic cá nhân khá nhất quán.

## 4. Đời thường / sở thích / thói quen
`UNSPECIFIED` trong nguồn đã audit.

## 5. Ngoại hình
Chỉ dùng character art/animation frame làm bằng chứng trực quan; mô tả chi tiết chưa được chuẩn hóa trong source text giữ `UNSPECIFIED`.

## 6. Racing / vai trò narrative
Vai trò/entry chưa được nguồn đã audit xác nhận thêm; không suy career từ ngựa thật.

## 7. Quan hệ
Một senior/legend figure, xuất hiện trong Cinderella Gray và broader franchise. Tương phản tốt với kiểu leadership có cấu trúc của Rudolf.

## 8. Animation / media appearances
Chỉ đánh dấu continuity/tập khi cast credit, caption, dialogue, race card hoặc frame nguồn xác nhận; chưa có evidence bổ sung thì giữ `UNSPECIFIED`.

## 9. Trivia / lưu ý cho fanfic
- Không chuyển quan hệ/ngày tháng/thành tích của ngựa thật thành canon nhân vật.
- Profile ngắn phản ánh độ sâu của nguồn, không phải giấy phép bịa thêm personality.

## 10. Nguồn đã khai thác
- https://umamusu.wiki/Mr._C.B.
- https://umamusume.jp/character/

## 11. Trạng thái nghiên cứu
`FOUND / NORMALIZED / HIGH-DETAIL`
